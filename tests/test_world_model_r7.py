"""WM-0E-R7 structural proofs: seed derivation, collision audit, definition
sealing, exactly-once cells, two-world integrity, frozen engine/surface.
No acceptance seed is passed through a model here."""
import json
import os
...
def test_seed_derivation_deterministic_role_labelled_and_new():
    man = F.seed_manifest()
    assert man["indices"] == 50 and len(man["cells"]) == 300 and man["manifest_hash"] == F.seed_manifest()["manifest_hash"]
    a = F.derive_seed("N1", "world", 3); assert a == F.derive_seed("N1", "world", 3) != F.derive_seed("N1", "world", 4)
    assert F.derive_seed("N0", "feature_world", 0) != F.derive_seed("N3", "target_world", 0)   # control in the key
    ...
    prim = [c["primary_seed"] for c in man["cells"]]; assert len(set(prim)) == 300


def test_collision_audit_is_exhaustive_and_zero():
    aud = F.collision_audit(F.seed_manifest())
    checked = aud["prior_sets_checked"]
    for k in ("V0_dev_25", "V2_1_consumed_50", "R3_N3_dev_100", "R3_N3_dev_shadow_100", "R3_P0_dev_50", "R4_P0_dev_50",
        ...
        assert k in checked, k
    assert aud["count"] == 0, aud["collisions"]
    # the audit does catch a planted collision
    fake = {"cells": [{"control": "N2", "index": 0, "primary_seed": 1000}]}
    assert F.collision_audit(fake)["count"] >= 1


def test_definition_is_pure_and_commits_to_everything():
    d1 = F.define("TESTCOMMIT", 0.0, FROZEN_SURFACE); d2 = F.define("TESTCOMMIT", 0.0, FROZEN_SURFACE)
    assert d1.court_hash == d2.court_hash and d1.court_id == d2.court_id
    assert F.define("OTHER", 0.0, FROZEN_SURFACE).court_hash != d1.court_hash
    c = d1.canonical()
    assert c["court_version"] == "FINAL_SYNTHETIC_ACCEPTANCE_COURT_V0" and c["authority"] == "SYNTHETIC_RESEARCH_ONLY"
    ...
    assert c["geometry"]["expected_usable_and_train"]["N1"] == [1860, 501] or c["geometry"]["expected_usable_and_train"]["N1"] == (1860, 501)
    assert c["geometry"]["expected_usable_and_train"]["P0"] == (1860, 701)
    assert "independent evidence" in c["acceptance"]["family"] and "same" in c["acceptance"]["family"].lower() or "ONE two-independent-world" in c["acceptance"]["family"]
...
def test_budget_snapshot_carries_the_whole_history_without_touching_the_surface():
    ...
    assert d["ledger_hash"] == wm0e_r51_truth().budget_hash               # the frozen ledger, unchanged
    ...
    assert c["null_control_validation"] == 7 and c["model_family"] == 2 and c["hyperparameter_configuration"] == 0
    h = d["additional_history"]
    assert len(h["regression_infrastructure"]) == 4 and len(h["acceptance_sets_consumed"]) == 1
    assert sum(h["development_seed_sets_observed"].values()) == 425 and len(h["failed_court_sittings"]) == 4
    assert h["one_model_one_test"] is False and d["snapshot_hash"] == F.r7_budget_snapshot()["snapshot_hash"]
...
def test_run_cell_refuses_unexpected_control_or_index(tmp_path):
    d = F.define("TESTCOMMIT", 0.0, FROZEN_SURFACE)
...
def test_two_world_integrity_for_every_N0_and_N3_seed_without_running_models():
    """World-hash level proof on the actual R7 seeds: cheap (no pipeline)."""
    man = F.seed_manifest()
    for c in man["cells"]:
        if c["control"] not in ("N0", "N3"):
            continue
...
def test_judge_rules_per_control_and_never_pools():
    ...
    for ctl in ("N0", "N1", "N2", "N3", "N4"):
        k = 6 if ctl == "N3" else 0                       # N3 fails, others clean
    ...
    assert F.judge(cells2)["controls"]["P0"]["verdict"] == "FAIL"          # 39/50 fails, no rounding
