...
def test_arithmetic_failures_in_dispersion_are_named_refusals(world, monkeypatch):
    ...
    with pytest.raises(D.DispersionRefused, match="D0_FITTER: ZeroDivisionError"):
        D.fit_d0(z)
    monkeypatch.undo()
    # an extreme optimiser result must be refused by the FITTER, not merely by the tournament
    monkeypatch.setattr(D, "_nelder_mead", lambda *a, **k: (np.array([1e9, 0.5]), 1.0, True, 5))
    with pytest.raises(D.DispersionRefused, match="D1_ARITHMETIC.*optimiser result"):
