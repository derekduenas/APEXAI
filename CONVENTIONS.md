...
|---|---|---|---|---|
| 2026-09-11 | A-011 — clock semantics for due dates | Calendar is canonical, lake is diagnostic. `calibration_report.json` is scored against the calendar date (`clock: CALENDAR_CANONICAL`); the lake-clock scoring is kept as `calibration_report_lake_diagnostic.json` labelled `LAKE_DIAGNOSTIC`, citable for what the data holds, never for standing. Full text: `CONVENTIONS-AMENDMENT-A-011.md`. | A due date must be measured by a clock the system cannot influence. The lake clock is system-controlled: with the pull stopped, 906 past-due predictions read not-yet-due for 21 days and every penalty was avoided by silence. Third instance of the gate-checks-a-property-the-system-controls class. | **AFTER** the 2026-08-24 report and BEFORE any producer standing is cited: gp_rank 0.239 → 0.293, h3 0.242 → 0.269, momentum_rank 0.260 → 0.303 (lake → calendar); blind twin unchanged at 0.250. |
