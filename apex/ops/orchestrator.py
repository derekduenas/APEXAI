...
# ======================================================================
# DIGITALOCEAN IS APEX HOME. THE MAC IS DEVELOPMENT / ADMIN ONLY.
#
# Permanent doctrine, 2026-08-26. On 2026-08-25 the Mac produced 175
# battery incidents, 4 network-unreachable events and 75 progression
# stalls during RTH, and equity tape continuity was 0.8103 beside a
# PERFECT data-quality record. No required market-day process may
# depend on a laptop's uptime, power state, network, launchd, local
# storage, or a human being logged in.
#
# MAC_RUNTIME_AUTHORITY = NONE. The Mac roster is EMPTY by law, and a
# test asserts it stays that way: APEX must operate normally with the
# Mac powered completely off.
# ======================================================================
MAC_RUNTIME_AUTHORITY = "NONE"
DIGITALOCEAN_RUNTIME_AUTHORITY = "CANONICAL"

SYSTEMCTL = ("/usr/bin/sudo", "-n", "/usr/bin/systemctl", "start")


def default_roster(host: str = "cloud") -> list:
    """What APEX is supposed to be doing on a market day, on THIS host.

    The cloud roster is the whole organism. The mac roster is empty --
    not "small", empty -- because a development laptop holds no
    runtime authority.

    EdgeForge and BTC are ALWAYS-ON sidecars deliberately: tying
    research observation to a market-open scheduler is exactly what
    lost 2026-08-25, and a sidecar with a durable cursor can catch up
    after an outage instead of losing the day permanently."""
    if host not in HOSTS:
        raise OrchestratorViolation(
            f"unknown host {host!r}; a roster must know which machine "
            f"it is describing")
    if host == "mac":
        return []          # MAC_RUNTIME_AUTHORITY = NONE
    return [
        # ---- MARKET CRITICAL (apex-market.slice)
        ServiceSpec(name="equity-fabric",
                    phases_running=("PREOPEN", "SESSION_ARMED", "RTH"),
                    # the orchestrator TRIGGERS on the exchange
                    # calendar (systemd OnCalendar cannot express
                    # holidays or half-days); systemd then SUPERVISES
                    start_cmd=SYSTEMCTL + ("apex-equity-fabric.service",),
                    first_work_deadline_s=600, beat_stale_s=300,
                    work_stale_s=900, required_for_arm=True,
        ...
        ServiceSpec(name="btc-paper", phases_running=(), always_on=True,
                    start_cmd=SYSTEMCTL + ("apex-btc-paper.service",),
                    first_work_deadline_s=1800, beat_stale_s=1800,
                    work_stale_s=2400, supervised_by="systemd"),
        # ---- RESEARCH / SHADOW (apex-research.slice)
        ServiceSpec(name="edgeforge-observatory",
                    ...
                    work_stale_s=None, supervised_by="systemd"),
    ]
