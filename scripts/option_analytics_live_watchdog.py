#!/usr/bin/env python
"""INDEPENDENT PROGRESS WATCHDOG for option_analytics_live_runtime --
same discipline as scripts/frontier2_watchdog.py: reasons from the
...
string without re-deriving it via apex.governance.service_progress.
    ...
    python scripts/option_analytics_live_watchdog.py
...
STATE_PATH = Path("results/option_analytics/live/runtime/service_progress.json")


def check() -> dict:
    if not STATE_PATH.exists():
        return {"service": "option_analytics_live_runtime", "progress_status": "STOPPED",
    ...
    return {
        "service": record.get("service_name", "option_analytics_live_runtime"),
