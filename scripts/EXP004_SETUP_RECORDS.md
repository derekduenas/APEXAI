...
| ownership | research root, checkout and out owned by uid 112; **`dataset_view` owned by root (uid 0), mode 755, its entries root-owned mode 664**; runner root and venv owned by root, mode 755 |
...
**Claimed by sha256, for these artifacts only:** the three signed decisions
(`1eafbe1f…`, `28bc2859…`, `dd968c31…`), `allowed_signers` `ae30f60d…`, the
EXP-001B rev2 sealed result `4b23021d…`, the EXP-002 sealed result
`0c397d70…`, and the EXP-001B aborted-run forecasts `7a6cbe36…` — all
unchanged. Accounts 109/110/111 present, plus the new 112.

**Not claimed:** whole output directories. The record's directory listing is a
truncated first-three sample for orientation and establishes nothing about the
completeness of those directories; the field is renamed
`other_env_output_listing_sample_first3` to stop it reading as evidence.
