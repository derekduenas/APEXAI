"""PULSE-009 -- NKLA before/after, plus a downstream-consumer audit.
...
Writes results/pulse009_nkla_before_after.json
       results/pulse009_consumer_audit.json
"""
import json
import os
import re
...
def before_after():
    ...
    after = compose(subject="NKLA", snapshot=snapshot,
                    ...
                    universe_version="PULSE009_DIAGNOSTIC",
    ...
    contaminated = {n: r for n, r in rows.items()
                    if r["before_q"] == VALID and r["after_q"] != VALID
                    and "ingredient(s)" in (r["after_note"] or "")}
    artefact = {n: r for n, r in rows.items()
                if r["before_q"] == VALID and r["after_q"] != VALID
                and "ingredient(s)" not in (r["after_note"] or "")}
    doc = {
        "kind": "pulse009_nkla_before_after", "version": "NKLA_BEFORE_AFTER_V1",
    ...
    json.dump(doc, open(R + "/pulse009_nkla_before_after.json", "w"), indent=1)
...
def consumer_audit():
    ...
    doc = {"kind": "pulse009_consumer_audit", "version": "CONSUMER_AUDIT_V1",
    ...
    json.dump(doc, open(R + "/pulse009_consumer_audit.json", "w"), indent=1)
...
if __name__ == "__main__":
    before_after()
    consumer_audit()
    print("\nwrote results/pulse009_nkla_before_after.json and results/pulse009_consumer_audit.json")
