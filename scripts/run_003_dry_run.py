#!/usr/bin/env python
"""APEX-003-H1 DRY-RUN CERTIFICATION. A specification validator. NO credit.

Runs the full production path (build_gp_output, returns included for SHAPE
only) and answers seven conformance sections with numbers. Two runs must be
byte-identical: the report carries no timestamps (progress goes to stderr).

Registration is NOT complete: config still names APEX-002 and the APEX-003
protocol is unsigned. Both are HUMAN acts and are reported as PENDING, not
smuggled past. This script therefore builds its config by overriding
experiment identity IN MEMORY, transparently, for the in-sample-only dry run --
the in-sample period is unlocked and free, and no gated path is invoked.
...
PACKETS = Path("results/003_registration_packets.json")
PROTOCOL = REPO / "APEX-003-Protocol-Gross-Profitability.md"
DEC = 12
...
def main() -> int:  # noqa: C901
    ...
    # Transparent in-memory identity override for the unlocked in-sample dry
    # run. Registration (config flip + signature + pin) is a pending human act.
    data = copy.deepcopy(base.data)
    data["experiment"]["id"] = "APEX-003"
    cfg = type(base)(data=data, sources=base.sources)
    period = "in_sample"

    r.say("=" * 78)
    r.say("APEX-003-H1 -- DRY-RUN CERTIFICATION (gross profitability)")
    ...
    packet = next(p for p in json.loads(PACKETS.read_text())["results"]
                  if p["title"].startswith("APEX-003-H1"))
    proto_text = PROTOCOL.read_text()
    proto_hash = hashlib.sha256(proto_text.encode()).hexdigest()

    r.check("pre-registered packet exists and survived the screen",
            packet["screen_verdict"] == "SURVIVE",
            f"dossier {packet['dossier_hash'][:12]}")
    r.check("frozen protocol document drafted", PROTOCOL.exists())
    r.fact("protocol SHA-256 (unsigned draft)", proto_hash)
    r.check("protocol transcribes the packet's criteria",
            "t >= 2.92" in proto_text and "block size **20**" in proto_text
            and "decile 1 (TOP)" in proto_text)
    r.pend("operator signature on the protocol",
           "human act -- 'Registered/Author' lines are PENDING by design")
    r.pend("config registration (experiment.id -> APEX-003) + CONVENTIONS pin",
    ...
    r.check("no token authorises APEX-003",
            not (tok["present"] and "APEX-003" in (tok["authorises"] or "")),
            f"token present: {tok['present']}"
            + (f" -- authorises {tok['authorises']!r} (STALE APEX-002; delete "
               f"before writing 003's)" if tok["present"] else ""))
    wiring = certify_experiment(REPO, "APEX-003")
    r.check("execution-path isolation certifies", wiring == [], str(wiring or "clean"))
    r.say()
    r.data["governance"] = {"dossier_hash": packet["dossier_hash"],
                            "protocol_draft_sha256": proto_hash}
    ...
    total = int(pop.sum())
    scored_mask = pop & score.notna().to_numpy()
    ...
    # signal means BY DECILE -- signal values only, never returns
    ...
    r.check("D3 criteria load: t >= 2.92, mean IC positive, sign agreement",
    ...
    r.check("D1 attribution executes on GPOutput (INCIDENT-001 class)",
    ...
    r.say()
    r.pend("stale smoke-gate in run_validation.py",
           "reads APEX-001-era production_smoke.txt; fix at registration turn")
    ...
    for p_ in r.pending:
        r.say(f"    * {p_}")
    r.say()
    r.say("  Credit NOT spent (2/5). Holdout SEALED. Registration NOT complete.")
    r.say("=" * 78)
