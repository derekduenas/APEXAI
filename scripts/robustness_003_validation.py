#!/usr/bin/env python
"""APEX-003 section-9 DECLARED robustness on the recorded validation look.

Part of the SAME single evaluation the credit paid for -- the protocol declared
bootstrap + permutation + subperiod diagnostics before validation. The verdict
(FAILURE, t=2.525 < 2.92) is recorded and CANNOT change here; section 13 does
not read these numbers. Reproduction-gated: aborts unless the recomputed IC
series matches the recorded artifact at 1e-12.
"""
...
rec = json.loads(Path("results/validation_APEX-003.json").read_text())["raw"] \
    if "raw" in json.loads(Path("results/validation_APEX-003.json").read_text()) \
    else json.loads(Path("results/validation_APEX-003.json").read_text())
rec_ic = rec["ic_daily_newey_west"] if "ic_daily_newey_west" in rec else rec["raw"]["ic_daily_newey_west"]
...
# section-9 declared robustness: block 20, 2000, seed 20260807
b = R.moving_block_bootstrap_mean(ic, block_size=20, n_resamples=2000, seed=20260807)
perm = R.block_sign_permutation(ic, block_size=20, n_permutations=2000, seed=20260807)
sub_y = R.subperiod_means(ic, scheme="calendar_year")
out = {"reproduction": "MATCH", "verdict_unchanged": "FAILURE (recorded)",
       "bootstrap": b.as_dict(), "permutation": perm.as_dict(),
       "subperiod_calendar_year": sub_y.as_dict()}
Path("results/003_robustness.json").write_text(json.dumps(out, indent=2, sort_keys=True) + "\n")
...
print("DIAGNOSTIC ONLY -- the recorded FAILURE verdict is unchanged.")
