"""WM-0E-R7.1 court runner (FINAL_COURT_RUNNER_V0.1, courts/final_court.py). Modes:
...
ROOT = "/opt/apex-research/world-model-shadow"; OUT = ROOT + "/evidence/final_court_v1"; os.makedirs(OUT, exist_ok=True)
CREATION_TIME = 1757100000.0
...
if mode == "predeclare":
    ...
    assert F.NAMESPACE and F.NAMESPACE not in F.CONSUMED_NAMESPACES, "acceptance namespace must be minted and new"
    man = F.seed_manifest(); aud = F.collision_audit(man); snap = F.r7_budget_snapshot()
    json.dump({**man, "status": "SEALED_UNCONSUMED", "collision_audit": aud, "runner": F.RUNNER_VERSION}, open(OUT + "/seed_manifest.json", "w"), indent=1)
    ...
    json.dump({"definition": defn.canonical(), "court_hash": defn.court_hash, "status": "SEALED_BEFORE_OUTCOMES",
               "R7_1_ACCEPTANCE_SET_STATUS": "SEALED_UNCONSUMED", "R7_1_PRIOR_SEED_COLLISIONS": aud["count"]},
...
if mode == "open":
    ...
    marker = OUT + "/R7_1_COURT_OPENED.json"
    if os.path.exists(marker):
        raise F.CourtIntegrityFailure("court already opened; R7 seeds are CONSUMED")
    oom0 = oom_count(); t0 = time.time()
    import subprocess
    tree_hash = subprocess.run(["git", "-C", ROOT, "rev-parse", "HEAD^{tree}"], capture_output=True, text=True).stdout.strip()
    F._write_once(marker, {"R7_1_COURT_OPENED": True, "court_id": defn.court_id, "court_version": F.COURT_VERSION,
                           "court_hash": defn.court_hash, "code_commit": commit,
                           "R7_1_OPENING_COMMIT": tree_commit, "tree_hash": tree_hash,
                           "tree_differs_from_code_commit_by": "sealed predeclaration evidence only (no code)",
                           "runner": F.RUNNER_VERSION, "runner_sha256": hashlib.sha256(open(F.__file__, "rb").read()).hexdigest(),
    ...
    out = {"court_id": defn.court_id, "court_hash": defn.court_hash, "code_commit": commit, "precourt_tree_commit": tree_commit, "surface_hash": surface,
           ...
           "integrity_failure": invalid, "judgement": verdict_doc, "FINAL_SYNTHETIC_ACCEPTANCE_COURT_V1": status, "runner": F.RUNNER_VERSION,
           ...
           "cells": cells}
    json.dump(out, open(OUT + "/FINAL_SYNTHETIC_ACCEPTANCE_COURT_V1.json", "w"), indent=1, default=str)
    ...
    print("FINAL_SYNTHETIC_ACCEPTANCE_COURT_V1:", status, "| integrity:", invalid, "| oom", oom0, "->", out["oom_kill_after"], "| elapsed %.0fs rss %.0fMiB" % (out["elapsed_s"], out["resource"]["ru_maxrss_MiB"]), flush=True)
