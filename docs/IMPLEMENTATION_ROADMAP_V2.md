...
|---|---|---|
| GARCH volatility challenger | test recursively evolving variance against the existing `rv_30` scale law **and a simple EWMA baseline** | a separate frozen comparison with correct intraday clocks, units and horizon aggregation. Reference: `agent/src/quantlib/timeseries.py::fit_garch` (GARCH(1,1), constant mean, Gaussian innovations, via `arch`); `volatility.py` is Heston. Nothing adopted |
